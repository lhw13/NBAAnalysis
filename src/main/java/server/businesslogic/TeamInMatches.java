...
public class TeamInMatches {
	...
	public void clean() {// deal with dirty or null data
		...
		for (int i = 0; i < players.size(); i++) {// deal with dirty score
			PlayerInMatchesPO player = players.get(i);
			if (player.getScore() < 0)
				player.setScore((player.getHit() - player.getThirdHit()) * 2
						+ player.getThirdHit() * 3 + player.getFreeHit());
		...
		for (int j = 4; j < getScores().size(); j++)
			eplayTime += 1500;
		for (int i = 0; i < size; i++) {// deal with dirty play time
