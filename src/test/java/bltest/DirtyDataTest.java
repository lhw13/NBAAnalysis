...
public class DirtyDataTest {
	...
	public void testDirtyPlayerHit() {
		...
		for (int i = 0; i < matches.size(); i++) {
			...
			for (int j = 0; j < players1.size(); j++) {
				PlayerInMatchesPO player = players1.get(j);
				if (player.getScore() != (player.getHit() - player
						.getThirdHit())
						* 2
						+ player.getThirdHit()
						* 3
	...
	public void testDirtyPlayerShot() {
		...
		for (int i = 0; i < matches.size(); i++) {
			...
			for (int j = 0; j < players1.size(); j++) {
				PlayerInMatchesPO player = players1.get(j);
				if (player.getShot() < player.getHit()
						|| player.getFreeshot() < player.getFreeHit()
						|| player.getThirdshot() < player.getThirdHit()
						|| player.getShot() - player.getThirdshot() < player
								.getHit() - player.getThirdHit())
					System.out.println(player);
			}
			for (int j = 0; j < players2.size(); j++) {
				PlayerInMatchesPO player = players2.get(j);
	...
	public void testDirtyTotalRebound() {
		...
		for (int i = 0; i < matches.size(); i++) {
			...
			for (int j = 0; j < players1.size(); j++) {
				PlayerInMatchesPO player = players1.get(j);
				if (player.getTotalRebound() != player.getDefensiveRebound()
						+ player.getOffensiveRebound())
					System.out.println(player);
			}
			for (int j = 0; j < players2.size(); j++) {
				PlayerInMatchesPO player = players2.get(j);
	...
	public void testDirtyOthers() {
		...
		for (int i = 0; i < matches.size(); i++) {
			...
			for (int j = 0; j < players1.size(); j++) {
				PlayerInMatchesPO player = players1.get(j);
				if (player.getAssist() < 0 || player.getBlock() < 0
						|| player.getSteal() < 0 || player.getMiss() < 0
						|| player.getFoul() < 0)
					System.out.println(player);
			}
			for (int j = 0; j < players2.size(); j++) {
